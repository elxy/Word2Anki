...
class IndexBuilder(object):
    ...
    def __init__(self,
        ...
        self._mdx_db = os.path.basename(_filename) + ".mdx.db"
        self._mdd_db = os.path.basename(_filename) + ".mdd.db"
    ...
    def _make_mdx_index(self):
        ...
        tuple_list = [(item['key_text'], item['file_pos'], item['compressed_size'], item['decompressed_size'],
                       item['record_block_type'], item['record_start'], item['record_end'], item['offset'])
        ...
        c.executemany('INSERT INTO META VALUES (?,?)', [('encoding', self.meta['encoding']),
    ...
    def lookup_indexes(db, keyword, ignorecase=None):
        indexes = []
        if ignorecase:
            sql = u'SELECT * FROM MDX_INDEX WHERE lower(key_text) = lower("{}")'.format(keyword)
        else:
            sql = u'SELECT * FROM MDX_INDEX WHERE key_text = "{}"'.format(keyword)
